...
1. Для автоматического добавления IP-адресов основных и голосовых серверов в IPset, используйте:
```bash
./ipset-adder.sh auto
```
2. Для автоматического добавления IP-адресов основных и голосовых серверов в конкретный IPset лист, используйте режим `list` передав в качестве аргумента:
```bash
./ipset-adder.sh list <ip_list_name>
```
3. Если вы не хотите использовать IPset, а только сгенерировать списки в его формате, выполните:
```bash
