...
1. Для автоматического добавления IP-адресов основных и голосовых серверов в IPset (_лист `unblock`_), используйте:
```bash
./ipset-adder.sh auto
```
2. Для автоматического добавления IP-адресов основных и голосовых серверов в конкретный IPset лист, используйте режим `list` передав в качестве аргумента имя вашего IPset листа:
```bash
./ipset-adder.sh list <ip_list_name>
```
3. Если вы не хотите осуществлять импорт сразу в IPset, а только сгенерировать списки в его формате, выполните:
```bash
./ipset-adder.sh noipset
```
4. Запуск в интерактивном режиме:
```bash
./ipset-adder.sh
```
...
Ветки отличаются друг от друга тем, что:
- В `light-no-timeout` при добавлении в IPset список **НЕ** добавляется атрибут таймаута (_как и в ветке `master`_). Таким образом ветка ориентирована на [релизную версию](https://github.com/qzeleza/kvas/releases) КВАС'а.
- Ветка `light` же напротив, ориентирована на **актуальную** версию КВАС'а, в бете которой уже используются таймауты и отличается имя IPset списка, вместо прежнего `unblock` теперь `KVAS_LIST`.
